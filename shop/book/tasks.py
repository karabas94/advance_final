...
def send_new_book_to_shop_or_update():
    ...
    store_books = []
    while True:
        store_books.extend(response['results'])
        if response['next']:
            response = requests.get(response['next']).json()
        else:
            break

    store_book_ids = [book['id'] for book in store_books]

    Book.objects.exclude(id_in_store__in=store_book_ids).delete()

    for book_data in store_books:
